...
```
Risk = 0.25 × statistical   (Z-score + IQR + Isolation Forest)
     + 0.20 × behavioral    (KL divergence vs baseline)
     + 0.18 × velocity      (rolling window frequency)
     + 0.15 × geographic    (haversine distance from home)
     + 0.10 × temporal      (hour-of-day deviation)
     + 0.12 × relationship  (shared device/IP clusters)
```
...
| Graph | NetworkX |
| Statistics | scipy (KS), numpy, pandas |
| Frontend | React 18, Vite, vis-network |
| HTTP | axios |
| Deployment | Docker + Docker Compose |
| Streaming | In-memory queue (Kafka optional) |

---

## Resume Bullets

> **Adaptive Transaction Intelligence Engine** — Python, FastAPI, XGBoost, NetworkX, React, Docker
>
> • Engineered a real-time behavioral anomaly detection pipeline maintaining rolling entity profiles across 5 time windows (5m–30d), combining statistical deviation, velocity, temporal, geographic, and relationship signals into a weighted composite risk score.
>
> • Built a dynamic transaction graph (NetworkX) detecting coordinated fraud clusters — shared devices/IPs across unrelated accounts — with 2-hop relationship anomaly scoring.
>
> • Implemented concept drift detection (PSI, KS statistic) to identify evolving fraud patterns and signal when model retraining is warranted.
>
> • Developed an interactive React dashboard with live transaction feed, SHAP-backed signal explanations, entity graph visualization, and a what-if risk simulator for demonstrating system behavior.
